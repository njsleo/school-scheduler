...
# =====================================================================
    ...
    if st.session_state['exam_result'] is not None:
        st.success("✅ 完美处理完毕！Excel已新增【考场分卷统计】Sheet页。")
        ...
        with col1:
            ...
            with pd.ExcelWriter(output_ex, engine='openpyxl') as writer:
                ...
                if all_data_list:
                    ...
                    paper_stats = pd.crosstab(full_df['考场号'], full_df['科目'])
                    cols_in_order = [c for c in EDITOR_ORDER if c in paper_stats.columns]
                    paper_stats = paper_stats[cols_in_order]
                    paper_stats['该考场总卷数'] = paper_stats.sum(axis=1)
                    paper_stats.to_excel(writer, sheet_name="考场分卷清单")
