...
if app_mode == "📅 智能排课系统":
    st.title("📅 全校智能排课系统")
    st.info("排课逻辑已稳定，请直接上传授课计划表进行运算。")
    # (此部分建议保留你之前的排课代码，此处为节省空间展示考务重点)
...
# =====================================================================
    ...
    class ExamScheduler:
        ...
        def arrange(self):
            ...
            for zkz, info in student_slips.items():
                ...
                # 按时间排序准考条内的科目，方便学生查看
                sorted_exams = sorted(info['exams'], key=lambda x: x['时间'])
    ...
    def export_slips_to_word(slips_dict, per_page, exam_title):
        ...
        for i in range(0, len(student_list), per_page):
            ...
            for idx, info in enumerate(chunk):
                ...
                # 宽度分配 (A4一半宽度约 3.75英寸)
                widths = [Inches(0.65), Inches(1.3), Inches(1.0), Inches(0.4)]
                ...
                for ex in sorted(info['exams'], key=lambda x: x['时间']):
                    row_c = inner.add_row().cells
                    vals = [ex['科目'], ex['时间'], ex['考场'], ex['座位']]
    ...
    if up_exam:
        ...
        # --- 动态时间编辑窗口 ---
        st.markdown("### 📅 第二步：填写考试时间 (点击下方表格可直接修改)")
        # 自动获取当前表中的所有科目
        all_subs = set(['语文', '数学'])
    ...
    if st.session_state['exam_result'] is not None:
        ...
        if "预览" in view:
            ...
            for zkz, info in list(st.session_state['exam_slips'].items())[:20]: # 网页只预览前20个防止卡顿
                html += f"""
                <div style="width:340px; border:1px dashed #999; border-radius:8px; background:#fff; padding:10px; margin-bottom:10px;">
                    ...
                    <table style="width:100%; border-collapse:collapse; font-size:11px; text-align:center; border:1px solid #ddd;">
                        <tr style="background:#f2f2f2;"><th>科目</th><th>时间</th><th>考场</th><th>座号</th></tr>
                """
                for ex in sorted(info['exams'], key=lambda x: x['时间']):
                    html += f"<tr style='border-bottom:1px solid #eee;'><td style='padding:4px;'>{ex['科目']}</td><td>{ex['时间']}</td><td style='font-weight:bold;'>{ex['考场']}</td><td>{ex['座位']}</td></tr>"
                html += "</table></div>"
