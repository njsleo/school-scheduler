...
with st.sidebar:
    ...
    st.markdown("### 2. 晚修规则配置")
    # 把原本死板的年级选择，换成灵活的周日晚修开关
    include_sunday_evening = st.checkbox("✅ 开启周日晚修 (高一/高二/高三通用)", value=True)
    st.caption("💡 提示：勾选后，周日晚上第9、10节也会像平时一样安排正课和晚自习。取消勾选则周日全天休息。")
...
if uploaded_file:
    ...
    if st.button("🚀 根据当前设置生成课表", type="primary"):
        with st.spinner("正在计算... (请确保左侧的参数设置正确)"):
            scheduler = Scheduler(teacher_df, class_prefix, class_suffix, include_sunday_evening)
